#include "AnvilMetis.h"
typedef std::pair<int, int> BodyPair;

// Internal factor implementation. Included by AnvilSolver.cpp.
...
private:
	...
	void orderBodies(const Sparse& matrix)
	{
		...
		else
		{
			...
			Anvil_METIS_SetDefaultOptions(options);
			const int result = Anvil_METIS_NodeND(&count, m_metisOuter.data(), m_metisInner.data(), NULL, options, m_bodyOrder.data(), m_metisInverse.data());
			if(result != METIS_OK)
			{
				for(int body = 0; body < bodies; ++body)
				{
