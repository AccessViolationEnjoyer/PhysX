#ifndef ANVIL_SOLVER_H
#define ANVIL_SOLVER_H

#include "AnvilParallel.h"
#include "AnvilStorage.h"
#include "AnvilMath.h"
#include <chrono>
#include <cstdint>
#include <string>
...
struct Problem
{
	...
	ANVIL_FORCE_INLINE void cancelScalarContact()
	{
		contacts.pop_back();
	}
...
struct SolverStatistics
{
	...
	int stopReason = 0; // 0: iteration limit, 1: gradient, 2: improvement, 5: precision, 6: line search, 7: velocity tolerance.
...
struct Settings
{
	...
	// Solves also stop once the next Anvil step would change no body's linear or angular
	// velocity by more than these (0 disables). A host that stores single-precision poses
	// cannot represent smaller changes, which contacts at the edge of their friction cone or
	// of contact can otherwise chase for several iterations.
	double velocityTolerance = 0.0;
	double angularVelocityTolerance = 0.0;
	// Unilateral solves continue with interior-point steps once Anvil has run at least
	// interiorPointSwitch iterations with three consecutive short line-search steps, or
	// four times that many iterations (negative disables). They return to Anvil once
...
private:
	...
	friend SolveStatus::Enum solveAnvil(const Problem&, const Settings&, Result&, Workspace&, const Result*) noexcept;
	friend SolveStatus::Enum continueAnvil(const Problem&, const Settings&, Result&, Workspace&, const Result*) noexcept;
};
...
SolveStatus::Enum solveAnvil(const Problem& problem, const Settings& settings, Result& result, Workspace& workspace, const Result* previous = NULL) noexcept;
...
SolveStatus::Enum continueAnvil(const Problem& problem, const Settings& settings, Result& result, Workspace& workspace, const Result* previous = NULL) noexcept;
}
#endif
