#ifndef ANVIL_MUJOCO_ADAPTER_H
#define ANVIL_MUJOCO_ADAPTER_H
...
typedef void (*AnvilMujocoTaskFunction)(const mjModel*, mjData*, void*, int, int);
extern "C" MJAPI void mju_dispatch(const mjModel*, mjData*, AnvilMujocoTaskFunction, void*, int);
...
static void* prepareIsland(void* argument)
{
	...
	{
	const anvil::Clock::time_point prepareStart = anvil::Clock::now();
	...
	anvil::Problem& problem = task.problem;
		problem.timestep = model->opt.timestep;
		problem.inverseMass.resize(dofCount / 6);
		problem.massDiagonal.resize(dofCount);
		problem.freeBodyVelocity = anvil::Vector::Zero(dofCount);
		anvil::VectorStorage& rootMass = task.rootMass;
		rootMass.resize(dofCount);
		anvil::Result& previous = task.previous;
		...
		anvil::reserveStorage(problem.contacts, size_t(rowCount));
		problem.contacts.resize(rowCount);
		for(int i = 0; i < rowCount; ++i)
		{
			const int row = data->map_iefc2efc[rowStart + i];
			anvil::CompactContact& contact = problem.contacts[i];
		...
		anvil::prepareProblemFromColumnCounts(problem);
		task.preparationMs = anvil::elapsed(prepareStart);
...
{
	IslandTask& task = *static_cast<IslandTask*>(argument);
	anvil::solveAnvil(task.problem, task.settings, task.result, task.workspace, &task.previous);
	return NULL;
}
...
{
	const anvil::Clock::time_point globalPreparationStart = anvil::Clock::now();
	...
	profile.globalPreparationWallMs = anvil::elapsed(globalPreparationStart);
	const anvil::Clock::time_point islandTasksStart = anvil::Clock::now();
	...
	profile.islandTasksWallMs = anvil::elapsed(islandTasksStart);
	int iterations = 0;
	for(int i = 0; i < islandCount; ++i)
	{
		const anvil::Result& result = islands[i].result;
	...
	const anvil::Clock::time_point scatterStart = anvil::Clock::now();
	for(int i = 0; i < islandCount; ++i)
		scatterIsland(islands[i]);
	mj_mulJacTVec(model, data, data->qfrc_constraint, data->efc_force);
	profile.scatterWallMs = anvil::elapsed(scatterStart);
	return iterations;
}
