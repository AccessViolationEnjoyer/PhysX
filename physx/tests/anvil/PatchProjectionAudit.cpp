...
struct Answer
{
	...
	anvil::PatchProjectionResult result;
};

bool project(const Sample& sample, Answer& answer)
{
	const anvil::PatchProjectionInput input = {sample.normalCount, sample.tangentCount,
	...
	const anvil::PatchProjectionOutput output = {answer.normal.data(), answer.tangent.data(),
		answer.normalDiagonal.data(), answer.tangentDiagonal.data(), answer.tangentCoupling.data()};
	return anvil::projectPatch(input, output, answer.result);
}
