...
static void prepare(anvil::Problem& problem, int bodies, int contacts, int step)
{
	...
	for(int i = 0; i < contacts; ++i)
	{
		...
		contact.freeVelocity = anvil::Vec3(std::sin(double(i)), std::cos(double(i)), -0.2 - 0.001 * step);
...
static void prepareExtended(anvil::Problem& problem, int step)
{
...
static bool auditExtended()
{
	...
	for(int step = 0; step < 1060; ++step)
	{
		...
		anvil::SolveStatus::Enum status = anvil::solveAnvil(problem, settings, result, workspace);
		...
		status = status == anvil::SolveStatus::eSUCCESS ? anvil::continueAnvil(problem, settings, result, workspace, &result) : status;
		updates += result.rankUpdates;
		allocationAudit::enabled = false;
		allocations += allocationAudit::allocations;
		frees += allocationAudit::frees;
		if(status != anvil::SolveStatus::eSUCCESS || result.gradientResidual > 1.0e-7)
		{
...
int main()
{
	...
	for(int step = 0; step < 36; ++step)
	{
		...
		anvil::solveAnvil(problem, settings, result);
		anvil::solveAnvil(problem, settings, result, &result);
	}
	...
	for(int step = 0; step < 1000; ++step)
	{
		...
		allocationAudit::frees = 0;
		allocationAudit::enabled = true;
		const anvil::Result* previous = step % 2 && result.primal.size() == 6 * bodies ? &result : NULL;
		anvil::solveAnvil(problem, settings, result, previous);
		allocationAudit::enabled = false;
