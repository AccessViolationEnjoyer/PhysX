"""Compare Anvil against MuJoCo 3.13.0 on identical pyramidal equations.
...
BUILD = ROOT / 'physx/compiler/anvil'
OUT = BUILD / 'results'
EXE = BUILD / 'build/Release/AnvilBenchmark.exe'
...
def run_case(case, scale, warm):
    ...
    record = dict(case=case, scale=scale, warm=warm, bodies=len(masses), contacts=len(pairs),
                  ...
                  native_strict=strict, anvil=ours, factor_check=check)
    print(case, scale, warm, 'native', round(native['median_ms'], 4),
          'Anvil', round(ours['median_ms'], 4), 'velocity error', ours['linear_velocity_error'], flush=True)
    return record
