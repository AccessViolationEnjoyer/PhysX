// Individual cases on ten long conveyors, simulated by PhysX with PGS or Anvil.
...
{
public:
	PxReal speed = PxReal(cases::beltSpeed);
...
int main(int argc, const char* const* argv)
{
	if(argc < 2)
	{
		printf("CaseConveyor output-prefix [steps=2000] [dt=.01] [pgs|anvil] [threads=8] [position=4] [velocity=1] [surface-regularization=1e-2] [stiffening-depth=2e-5] [tolerance=1e-8] [belt-ramp=1]\n");
		return 1;
	}
	...
	const double ramp = argc > 11 ? atof(argv[11]) : cases::beltRamp;
	...
	for(int step = 0; step < steps; ++step)
	{
		...
		belts.speed = PxReal(cases::beltSpeedAt((step + 1) * double(timestep), ramp));
