...
// Belts accelerate from rest to full speed over this many seconds by default, as real
// conveyors do, so friction carries the loads up to speed without sliding.
const double beltRamp = 1.0;

// Belt speed at a time, accelerating linearly from rest over ramp seconds (0: at full speed).
inline double beltSpeedAt(double time, double ramp)
{
	return ramp > 0.0 && time < ramp ? beltSpeed * time / ramp : beltSpeed;
}
