...
usage: python PlotCases.py <results-dir> [title=...] name=label [name=label ...]

Each name refers to `<name>-steps.csv` in the results directory; the tote scene writes the
same step columns. Bars show the mean step time over steps 100-2000, after the cases have
reached belt speed.
"""
import sys
...
def main():
    ...
    titles = [label for name, label in series if name == 'title']
    series = [(name, label) for name, label in series if name != 'title']
    title = titles[0] if titles else '2,000 cases on ten conveyors (10 ms steps, 8 workers)'
    means = []
    ...
    axis.set_title(title, loc='left', fontsize=12, color=INK)
