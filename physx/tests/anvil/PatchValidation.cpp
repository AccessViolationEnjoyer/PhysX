...
// active sets. The reference contains no patch projection or Anvil curvature.
...
int main()
{
	...
	for(int fixture = 0; fixture < 48; ++fixture)
	{
		...
		if(!referencePatch(problem, reference) || solveAnvil(problem, settings, result, workspace) != SolveStatus::eSUCCESS)
		{
		...
		if(!referencePatch(problem, reference) || solveAnvil(problem, settings, result, workspace, &result) != SolveStatus::eSUCCESS || (reference - toEigen(result.impulse)).lpNorm<Eigen::Infinity>() > 1.0e-8)
		{
			return 1;
		}
	}
	// Larger/changing groups exercise the two-vector Hessian and sparse updates;
	// every Anvil direction is independently checked against sparse J^T*H*J.
	for(int fixture = 0; fixture < 48; ++fixture)
	{
		preparePatch(problem, fixture, 12, 3 + fixture % 7, 4);
		prepareProblem(problem);
		if(solveAnvil(problem, settings, result, workspace) != SolveStatus::eSUCCESS || computeResidual(problem, result.impulse) > 1.0e-8)
		{
