...
static int countContactPairs(const mjData* data)
{
	...
	pairs.clear();
	anvil::reserveStorage(pairs, size_t(data->ncon));
...
int main(int argc, const char* const* argv)
{
	...
	mj_step1(model, data);
	anvil::validateMujocoModel(model, data, bodies.size());
	...
	printf("MEASUREMENT solve_ms is the complete solver stage: Anvil adapter preparation, fused island tasks and scatter, or native mj_fwdConstraint. Isolated Anvil preparation/solve wall times are unavailable (-1); island_tasks_wall_ms includes dispatch and joins.\n");
	printf("%s: %zu bodies, dt %.6g s, cap %d, %d workers, impedance %.6g; warm start on\n",
		prototype ? "Prototype Anvil" : "MuJoCo Newton", bodies.size(), model->opt.timestep,
