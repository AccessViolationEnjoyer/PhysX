...
The Anvil timings below describe the earlier MuJoCo-hosted benchmark. Current native
...
The requested test reproduces the thin-sheet problem in PGS. Both Anvil implementations
keep the sheets intact at a 10 ms timestep. No Anvil performance run reaches its
...
| Our incremental Anvil, cap 100 | 10 ms | 12.17 ms | 12.17 ms | 0.001473 mm |
...
adjacencies in PGS 16+2 and 62 in PGS 98+2 intrude through that thickness. Neither Anvil
...
Our Anvil is about **7.2% slower than native MuJoCo** on whole-run average full-step time
in this scene. It costs about **45% of the 1 ms PGS control** per simulated second and has
substantially smaller overlap. Both Anvil modes exceed a 10 ms real-time budget slightly
...
island iteration count was 4.91 for native Anvil and 5.04 for our Anvil. Each engine had
...
**The PhysX snippet uses stock PGS. Our Anvil still runs through the MuJoCo pipeline;
it is not integrated into PhysX yet.** Its full-step timing includes MuJoCo collision
processing, contact-to-prototype conversion, task dispatch, the Anvil solve and integration.
...
native Anvil. That is not used in the reported results. The earlier zero-margin diagnostic
runs are also excluded. The two engines' collision and friction models differ; these results
do not isolate PGS versus Anvil on identical PhysX constraints. The two Anvil modes do
...
Relative to `physx/compiler/anvil/results/pallet/comparison`:
...
- `anvil.mp4`: replay of all five conveyors using recorded Anvil poses.
- `anvil-final.png`: overview after 20 simulated seconds.
- `pgs-final.png` / `anvil-lane-final.png`: matching close views.
