...
struct ComparisonContext
{
	...
	PxScene* scene(ConveyorModifier& modifier, ImpulseRecorder& recorder)
	{
		...
		desc.anvilRegularization = settings.regularization;
...
int main(int argc, char** argv)
{
	if(argc < 4)
	{
		std::fprintf(stderr, "Usage: AnvilFrictionComparison <all|flat|incline|conveyor|static|cylinder|capsule|stack> "
			"<pgs|anvil> <trajectory.csv|-> [speed=.2] [steps=300] [threads=8] [regularization=1e-4] "
	...
	if((!all && selected < 0) || (std::strcmp(argv[2], "pgs") != 0 && std::strcmp(argv[2], "anvil") != 0))
	{
		return 2;
	}
	ComparisonSettings settings;
	settings.solver = std::strcmp(argv[2], "pgs") == 0 ? PxSolverType::ePGS : PxSolverType::eANVIL;
	...
	if(valid)
	{
		...
		std::printf("configuration timestep=%.9g threads=%u position_iterations=%u velocity_iterations=%u "
			"anvil_iterations=100 anvil_tolerance=1e-8 friction_every_iteration=1 default_contact_offsets=1\n",
