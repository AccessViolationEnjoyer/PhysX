...
#include "anvil/DyAnvilConstraintPrep.h"
#include "anvil/core/AnvilSolver.h"
...
static void initializeProblem(anvil::Problem& problem)
{
...
static AnvilJointSettings makeSettings(const PxSolverBodyData& body0, const PxSolverBodyData& body1,
...
static double hardJointFreeVelocity(const Px1DConstraint& row, const AnvilJointSettings& settings)
{
...
static double solveImpulse(anvil::Problem& problem)
{
	...
	check(anvil::solveAnvil(problem, settings, result, workspace) == anvil::SolveStatus::eSUCCESS, "solve scalar joint problem");
...
static void testSprings()
{
	...
	const AnvilJointSettings settings = makeSettings(body0, body1, initialVelocities);
	for(PxU32 acceleration = 0; acceleration < 2; ++acceleration)
	{
		...
		AnvilJointRows output;
		anvil::Problem problem;
		initializeProblem(problem);
		prepareAnvilJoint(constraint, body0, body1, 0, 1, NULL, settings, problem, output);
...
static void testBoundsAndReferences()
{
	...
	prepareAnvilJoint(constraint, falling, body1, 0, -1, NULL, settings, problem, output);
...
static void testWhiteningAndWriteback()
{
	...
	initializeProblem(problem);
	AnvilJointRows output;
	prepareAnvilJoint(constraint, body0, body1, 0, -1, &writeback, settings, problem, output);
	...
	writebackAnvilJoints(output, problem, result);
...
static void testSlerpDrives()
{
	...
	for(PxU32 acceleration = 0; acceleration < 2; ++acceleration)
	{
		for(PxU32 capped = 0; capped < 2; ++capped)
		{
			...
			anvil::Result result;
			anvil::Workspace workspace;
			anvil::Settings solveSettings;
			solveSettings.tolerance = 1.0e-12;
			check(anvil::solveAnvil(problem, solveSettings, result, workspace) == anvil::SolveStatus::eSUCCESS, "solve SLERP problem");
...
int main()
{
	...
	foundation->release();
	printf("Anvil joint adapter: %d failures\n", failures);
	return failures ? 1 : 0;
}
