...
static int validateMixedConstraints()
{
	...
	for(int fixture = 0; fixture < 6; ++fixture)
	{
		...
		prepareProblem(problem);
		Result cold;
		solveAnvil(problem, settings, cold);
		problem.freeVelocity *= 1.01;
		Result warm;
		solveAnvil(problem, settings, warm, &cold);
...
int main(int argc, char** argv)
{
	...
	if(argc < 2)
	{
		std::fprintf(stderr, "Usage: AnvilBenchmark input [seed|-] [solution|-] [repeats=8] [iterations=100] [check=0] [islands=1] [threads=1]\n"
			"       AnvilBenchmark cable links steps [trajectory]\n");
		return 2;
	}
	Settings settings;
	...
	for(int repeat = 0; repeat < repeats; ++repeat)
	{
			...
			for(int island = 0; island < islands; ++island)
				solveAnvil(problem, settings, results[island], warm ? &previous : NULL);
