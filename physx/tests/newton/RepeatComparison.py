...
def run(batches, repeats, case, scale, output):
    ...
    for batch in range(batches):
        ...
        record = dict(native=native, newton=ours, error=error, ratio=ours['median_ms']/native['median_ms'])
        records.append(record)
        print(batch, 'Newton', ours['median_ms'], 'MuJoCo', native['median_ms'], 'ratio', record['ratio'], flush=True)
    return records
...
def main():
    ...
    result = dict(case=options.case, scale=options.scale, affinity_mask=mask, batches=options.batches, repeats=options.repeats, records=records,
                  newton_ms=statistics.median(r['newton']['median_ms'] for r in records),
    ...
    print('Median batch times:', result['newton_ms'], result['native_ms'])
