...
static bool compare(const newton::Problem& problem, const newton::Settings& settings, const newton::Result* seed, newton::Workspace& continuedWorkspace, newton::Workspace& referenceWorkspace, newton::Result& continued, newton::Result& reference, int& factors, int& updates, int& reuse, double& maximum)
{
	...
	if(!check(actual == newton::SolveStatus::eSUCCESS && expected == actual, "continued/reference success"))
	{
		return false;
	}
...
static bool targetSequences()
{
	...
	for(int fixture = 0; fixture < 12; ++fixture)
	{
		...
		if(!check(newton::solveNewton(problem, settings, continued, continuedWorkspace) == newton::SolveStatus::eSUCCESS, "initial ordinary solve"))
		{
			return false;
		}
		earlier = continued;
...
static bool invalidation()
{
	...
	if(!check(newton::solveNewton(problem, settings, result, workspace) == newton::SolveStatus::eSUCCESS, "fixed impulse initial"))
	{
		return false;
	}
	problem.setScalarBounds(0, -maximumImpulse, maximumImpulse);
	if(!check(newton::continueNewton(problem, settings, result, workspace, &result) == newton::SolveStatus::eSUCCESS && result.factorizations > 0 && std::abs(result.primal[0] - 1.0 / 1.05) < 1.0e-12, "equality shortcut cannot reuse old fixed-row Hessian"))
	{
		return false;
	}
	problem.freeVelocity[0] = -1.2;
	if(!check(newton::continueNewton(problem, settings, result, workspace, &result) == newton::SolveStatus::eSUCCESS && result.factorizations == 0 && result.reusedFactors > 0, "unchanged equality factor reused for target change"))
	{
		return false;
	}
	problem.freeVelocity[0] = -1.3;
	if(!check(newton::continueNewton(problem, settings, result, workspace) == newton::SolveStatus::eSUCCESS, "changed target succeeds"))
	{
		return false;
	}
	newton::Workspace moved(std::move(workspace));
	problem.freeVelocity[0] = -1.4;
	if(!check(newton::continueNewton(problem, settings, result, moved, &result) == newton::SolveStatus::eSUCCESS && result.factorizations == 0, "workspace move preserves valid continuation"))
	{
		return false;
	}
	if(!check(newton::solveNewton(problem, settings, result, moved) == newton::SolveStatus::eSUCCESS && result.factorizations > 0, "ordinary solve always resets numerics"))
	{
		return false;
	}
	const std::uint64_t generation = problem.preparationGeneration;
	...
	if(!check(newton::continueNewton(problem, settings, result, moved) == newton::SolveStatus::eSUCCESS && result.factorizations > 0, "repaired preparation refactors"))
	{
		return false;
	}
