...
The current Newton prototype remains faster than native MuJoCo on a single connected pile,
but its absolute cost grows sharply with island size. Neither implementation currently
parallelizes the Newton solve within an island. This benchmark does not change the solver
...
into retained Newton storage, dispatch, complete solving, joins and result writeback.
...
host simulation work, excluding diagnostics and file I/O. No number here is a PhysX Newton
simulation-step measurement; that integration remains pending.

From the repository root:

```powershell
cmake --build physx/compiler/newton/build --config Release --target NewtonPileBenchmark
python physx/tests/newton/pile/ComparePile.py --sizes 125 500 1000 --steps 201 --repeats 1 --threads 8 --snapshot-repeats 5 --output physx/compiler/newton/results/pile/shared-snapshots-8
python physx/tests/newton/pile/ComparePile.py --sizes 500 --steps 201 --repeats 1 --threads 1 --snapshot-repeats 5 --output physx/compiler/newton/results/pile/shared-snapshots-1
python physx/tests/newton/pile/ComparePile.py --sizes 500 --steps 201 --repeats 1 --threads 8 --snapshot-repeats 5 --output physx/compiler/newton/results/pile/shared-snapshots-8-control
python physx/tests/newton/pile/ComparePile.py --sizes 500 1000 --steps 30 --warmup 10 --repeats 1 --threads 8 --output physx/compiler/newton/results/pile/pilot-2g
```
...
`physx/compiler/newton/results/pile/comparison-summary.json` joins the runs and source hashes.
...
No Newton kernel, tolerance, factor-update policy, or SDK source was changed for this benchmark.
