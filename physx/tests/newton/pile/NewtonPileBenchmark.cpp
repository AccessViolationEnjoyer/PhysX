...
int main(int argc, const char* const* argv)
{
	...
	{
		printf("NewtonPileBenchmark --write-scene output.xml width depth layers\n"
			"NewtonPileBenchmark scene.xml output-prefix prototype|mujoco [steps=1000] [threads=8] [auditEvery=0] [profile=0] [snapshotRepeats=0]\n");
		return 1;
	}
	...
	for(int step = 0; step < steps; ++step)
	{
		...
		const double solveMs = newton::elapsed(solveStart);
		const newton::Clock::time_point diagnosticStart = newton::Clock::now();
		...
		const double diagnosticMs = newton::elapsed(diagnosticStart);
		mj_sensorAcc(model, data);
		mj_checkAcc(model, data);
		mj_Euler(model, data);
		const double stepMs = newton::elapsed(stepStart) - diagnosticMs;
