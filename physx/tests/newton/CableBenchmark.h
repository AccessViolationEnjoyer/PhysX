	...
	const Vector& velocity, double dt, double anchorRegularization, bool gravity)
{
	...
	for(int i = 0; i < links; ++i)
	{
		...
		const Vec3 arm = rotation * Vec3(length, 0.0, 0.0);
		...
		const Vec3 angleError = rotation * Vec3(relative.vec() * (sine > 1.0e-12 ? 2.0 * std::atan2(sine, relative.w()) / sine : 2.0));
		for(int angular = 0; angular < 2; ++angular)
		{
			...
			c.freeVelocity += c.jacobian[0] * problem.freeBodyVelocity.segment<6>(6 * i);
			if(i)
				c.freeVelocity += c.jacobian[1] * problem.freeBodyVelocity.segment<6>(6 * (i - 1));
...
static int benchmarkCable(int links, int steps, const char* path, const Settings& settings)
{
	...
	for(int step = 0; step < steps; ++step)
	{
		...
		response.noalias() = problem.jacobian.transpose() * result.impulse;
		velocity = problem.freeBodyVelocity + result.primal;
		for(int i = 0; i < links; ++i)
		{
			...
			if(angle > 0.0)
				rotations[i] = (Eigen::Quaterniond(Eigen::AngleAxisd(angle, angular / angle)) * rotations[i]).normalized();
		}
		...
		finalResidual = std::max((result.primal - response).lpNorm<Eigen::Infinity>(),
			(problem.jacobian * result.primal + problem.freeVelocity + problem.regularization.cwiseProduct(result.impulse)).lpNorm<Eigen::Infinity>());
		maximumResidual = std::max(maximumResidual, finalResidual);
		double anchorError = 0.0;
		for(int i = 0; i < links; ++i)
		{
			const Vec3 anchor = i ? Vec3(positions[i - 1] + rotations[i - 1] * Vec3(0.05, 0.0, 0.0)) : Vec3(0.05, 0.0, 0.0);
			anchorError = std::max(anchorError, (positions[i] - anchor).norm());
		}
		maxAnchorError = std::max(maxAnchorError, anchorError);
		peakDeflection = std::max(peakDeflection, -positions.back().y());
		maxIterations = std::max(maxIterations, result.iterations);
		if(!velocity.allFinite() || !std::isfinite(finalResidual) || positions.back().norm() > 1000.0)
		{
			std::printf("CABLE_DIVERGED,step=%d,time=%.9g,tip=%.12g,anchor_error=%.12g,residual=%.12g,solve_ms=%.6f\n",
				step, (step + 1) * dt, -positions.back().y(), anchorError, finalResidual, result.elapsedMs);
			throw std::runtime_error("Cable simulation diverged");
		}
		trajectory << std::setprecision(12) << step << ',' << (step + 1) * dt << ',' << -positions.back().y() << ','
	...
	std::printf("CABLE_RESULT,links=%d,steps=%d,threads=%d,dt=%.9g,warm=%d,anchor_regularization=%.9g,tip=%.12g,beam_reference=%.12g,peak_tip=%.12g,anchor_error=%.12g,final_residual=%.12g,max_residual=%.12g,median_step_ms=%.6f,median_solve_ms=%.6f,max_iterations=%d\n",
		links, steps, threads, dt, int(warm), anchorRegularization, -positions.back().y(), expected, peakDeflection,
