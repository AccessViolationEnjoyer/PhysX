...
static NativeNewtonProfiler profiler;
...
struct TestContext
{
	...
	PxScene* scene(PxVec3 gravity = PxVec3(0.0f, -9.81f, 0.0f))
	{
		...
		desc.newtonRegularization = regularization;
...
static void massRatio(TestContext& context, PxMaterial& material)
{
	...
	if(context.solver == PxSolverType::eNEWTON)
	{
		check(light->getGlobalPose().p.y > .48f && heavy->getGlobalPose().p.y > 1.46f, "Newton supports heavy body on light body");
...
int main(int argc, char** argv)
{
	...
	printf("Newton profile islands=%d mean_iterations=%.9g min_iterations=%d max_iterations=%d mean_island_ms=%.9g mean_prepare_ms=%.9g mean_solve_ms=%.9g iteration_limits=%d\n",
