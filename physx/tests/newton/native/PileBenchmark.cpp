...
int main(int argc, char** argv)
{
	...
	const PxSolverType::Enum solver = argc > 3 && std::strcmp(argv[3], "pgs") == 0 ? PxSolverType::ePGS : PxSolverType::eNEWTON;
	...
	if(solver == PxSolverType::eNEWTON)
	...
	printf("%s pile: %dx%dx%d, %zu bodies, dt=0.01, workers=%u, regularization=%.9g, normal offsets\n",
		solver == PxSolverType::eNEWTON ? "Newton" : "PGS", width, depth, layers, bodies.size(), workers, double(desc.newtonRegularization));
	bool finite = true;
