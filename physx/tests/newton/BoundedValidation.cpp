...
static bool referenceBounds(const newton::Problem& problem, Eigen::VectorXd& impulse)
{
	...
	for(int i = 0; i < count; ++i)
	{
		...
		upper[i] = contact.hasScalarBounds() ? problem.bounds(contact).upper : newton::MAX_IMPULSE;
	}
	int combinations = 1;
	for(int i = 0; i < count; ++i)
	{
	...
	for(int candidate = 0; candidate < combinations; ++candidate)
	{
		...
		for(int row = 0; row < count; ++row)
		{
			...
			else
			{
				...
				valid = valid && trial[row] != newton::MAX_IMPULSE && trial[row] != -newton::MAX_IMPULSE;
			}
		}
		if(!valid)
		{
